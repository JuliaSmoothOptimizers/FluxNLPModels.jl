...
function loss_and_accuracy(data_loader, model, device, T) #TODO fix this 
  acc = T(0)
  ls = T(0.0f0)
  num = T(0)
...
function cb(
  ...
  solver,
  stats,
  # train_loader, ##TODO remove and use NLP.train_loader
  # test_loader,
  device,
  ...
  batch_end = data.batch_start + data.batch_size
  minibatch = nlp.train_loader[(data.batch_start):min(batch_end, end)]

  nlp.current_training_minibatch = device(minibatch) # move to cpu or gpu

  # data.batch_size= min(1000, batchsize +2) #TODO update batch_size
  if batch_end < length(nlp.train_loader)
...
function train_FluxNlPModel_R2(;
  ...
  minibatch = data_train[batch_start:min(batch_end, end)]
  minibatch_test = data_test[batch_start:min(batch_end, end)]
  ...
  batch_start = batch_end < length(data_train) ? batch_end : 1 #TODO test this 

  stochastic_data = StochasticR2Data(0, 0, batch_size, batch_start, atol, nothing) # data.i =0
...
#     # end
#     optimize_step!(model, gradient, learning_rate)
#     # batch_size= min(1000, batchsize +2) #TODO update batch_size
#     batch_start = batch_end > length(data_train) ? batch_end : 1
# end
