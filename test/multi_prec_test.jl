...
@testset "FluxNLPModel ill-instanciation checks" begin
  try 
    FluxNLPModel(model16_cpu,loader_cpu,loader_cpu;Formats=[f16,gpu])
    @test false
  catch
    @test true
  end
  try 
    FluxNLPModel(model16_cpu,loader_gpu,loader_cpu)
    @test false
  catch
    @test true
  end
  try 
    FluxNLPModel(model16_cpu,loader_cpu,loader_gpu)
    @test false
  catch
    @test true
  end
